...
import (
	...
	"github.com/thanhenti/bepaylot/internal/agent/events"
...
// New builds an Agent.
func New(st *postgres.Store, reg *llm.Registry, tr *tools.Registry, sk *skills.Service, acfg config.AgentCfg, lcfg config.LLM, log *slog.Logger) *Agent {
...
type RunStats struct {
	...
	LatencyMS    int
	SkillsFound  []string
	Detail       map[string]any // stored with the run; see types.AgentRun.Detail
}
...
func (a *Agent) persistUser(ctx context.Context, sessionID uuid.UUID, text string) (types.Message, error) {
	pctx, cancel := context.WithTimeout(context.WithoutCancel(ctx), 20*time.Second)
	defer cancel()
	msg := types.Message{
		SessionID: sessionID,
		Role:      types.RoleUser,
		Blocks:    []types.ContentBlock{{Type: types.BlockText, Text: text}},
	}
	if err := a.store.Messages.Append(pctx, &msg); err != nil {
		return types.Message{}, fmt.Errorf("persist user message: %w", err)
	}
	return msg, nil
}
...
func (a *Agent) runTurn(ctx context.Context, ar *activeRun, in RunInput, sink events.Sink) (RunOutput, error) {
	...
	fail := func(err error) (RunOutput, error) {
		...
		if len(blocks) == 0 {
			blocks = []types.ContentBlock{{Type: types.BlockText, Text: ""}}
		}
		assistantMsg := types.Message{
			SessionID:  sess.ID,
			Role:       types.RoleAssistant,
			StopReason: stats.StopReason,
			Usage:      types.Usage{InputTokens: stats.InputTokens, OutputTokens: stats.OutputTokens},
			Blocks:     blocks,
		}
	...
	if len(blocks) == 0 && runErr != nil {
		assistantMsg.Blocks = []types.ContentBlock{{Type: types.BlockText, Text: ""}}
	}
...
func runDetail(stopReason string, maxTokens int, temperature *float32, blocks []types.ContentBlock, skills []string, historyMessages int, ar *activeRun) map[string]any {
	d := map[string]any{
	...
	for _, b := range blocks {
		if b.Type == types.BlockToolUse && b.ToolName != "" {
			tools[b.ToolName]++
		}
	}
	if len(tools) > 0 {
