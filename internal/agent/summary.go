...
func (a *Agent) maybeSummarize(sess types.Session, msgCount int, provider llm.Provider, model string) {
	...
	go func() {
		...
		if _, err := a.store.Sessions.Update(ctx, sess.ID, postgres.UpdateParams{Summary: &summary}); err != nil {
