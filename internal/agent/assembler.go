...
func (a *assembler) messageStart(messageID, model, sessionID string, inputTokens int) {
	...
	a.emit(events.Event{Kind: events.KindMessageStart, MessageID: messageID, Model: model, SessionID: sessionID, Usage: types.Usage{InputTokens: inputTokens}})
}
...
func (a *assembler) toolEnd(name, response string, isErr bool) {
	...
	a.blocks = append(a.blocks, types.ContentBlock{
		Idx:       a.nextOrder(),
...
func (a *assembler) emitStop() {
	...
	a.emit(events.Event{
		...
		Usage: types.Usage{InputTokens: a.inputTokens, OutputTokens: a.outputTokens},
	})
...
func (a *assembler) closeLocked() {
	...
	switch ob.typ {
	case "text":
		a.blocks = append(a.blocks, types.ContentBlock{Idx: order, Type: types.BlockText, Text: ob.text.String()})
	case "thinking":
		a.blocks = append(a.blocks, types.ContentBlock{Idx: order, Type: types.BlockThinking, Thinking: ob.text.String()})
	case "tool_use":
		input := parseToolArgs(ob.args.String())
		a.blocks = append(a.blocks, types.ContentBlock{
			Idx: order, Type: types.BlockToolUse,
...
func (a *assembler) result(defaultFinish string) ([]types.ContentBlock, runStats) {
	a.mu.Lock()
	defer a.mu.Unlock()
	blocks := make([]types.ContentBlock, len(a.blocks))
...
func sortBlocksByIdx(b []types.ContentBlock) {
	for i := 1; i < len(b); i++ {
