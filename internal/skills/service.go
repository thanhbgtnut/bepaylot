...
func NewService(repo *postgres.SkillsRepo, embedder retrieval.Embedder, dir string, log *slog.Logger) *Service {
...
func (s *Service) Sync(ctx context.Context) (SyncResult, error) {
	...
	for _, d := range discovered {
		id, changed, err := s.repo.Upsert(ctx, types.Skill{
			Slug:         d.Slug,
			Name:         d.Name,
...
func (s *Service) List(ctx context.Context) ([]types.Skill, error) {
...
func (s *Service) Retrieve(ctx context.Context, queryText string, k int) []postgres.SkillMatch {
