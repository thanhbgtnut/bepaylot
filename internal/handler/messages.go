...
import (
	...
	"github.com/thanhenti/bepaylot/internal/handler/sse"
...
func (h *Handlers) resolveSession(ctx context.Context, user types.User, req dto.MessagesRequest, provider, model, firstText string) (types.Session, error) {
	if req.Metadata != nil && req.Metadata.SessionID != "" {
		...
		if sess.UserID != user.ID {
			return types.Session{}, errSessionForbidden
		}
		return sess, nil
	}

	title := sessionTitle(firstText, 60)
	return h.Store.Sessions.Create(ctx, postgres.CreateParams{
		UserID:   user.ID,
		Title:    title,
		Provider: provider,
		Model:    model,
