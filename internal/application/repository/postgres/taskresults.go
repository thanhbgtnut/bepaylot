...
// TaskResultsRepo persists types.TaskResult, independent of the message
...
func (r *TaskResultsRepo) Upsert(ctx context.Context, sessionID uuid.UUID, taskKey, title string, result map[string]any) (types.TaskResult, error) {
	resultJSON, err := cleanJSON(result)
	if err != nil {
		return types.TaskResult{}, fmt.Errorf("task_results.Upsert marshal: %w", err)
	}
	var t types.TaskResult
	...
	if err != nil {
		return types.TaskResult{}, fmt.Errorf("task_results.Upsert: %w", err)
	}
	return t, nil
}
...
func (r *TaskResultsRepo) ListBySession(ctx context.Context, sessionID uuid.UUID) ([]types.TaskResult, error) {
