...
func (r *MessagesRepo) Append(ctx context.Context, m *types.Message) (err error) {
...
func (r *MessagesRepo) ListBySession(ctx context.Context, sessionID uuid.UUID, afterSeq, limit int) ([]types.Message, error) {
	...
	byID := map[uuid.UUID]*types.Message{}
	for rows.Next() {
		var m types.Message
		var usageJSON []byte
