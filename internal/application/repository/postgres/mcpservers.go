...
func (r *MCPServersRepo) List(ctx context.Context) ([]types.MCPServer, error) {
...
func (r *MCPServersRepo) Get(ctx context.Context, name string) (types.MCPServer, error) {
	var s types.MCPServer
	...
	if errors.Is(err, pgx.ErrNoRows) {
		return types.MCPServer{}, ErrNotFound
	}
	if err != nil {
		return types.MCPServer{}, fmt.Errorf("mcp_servers.Get: %w", err)
	}
	return s, nil
}

// Upsert inserts or replaces a server by name.
func (r *MCPServersRepo) Upsert(ctx context.Context, name, transport string, enabled bool, spec map[string]any) (types.MCPServer, error) {
	specJSON, err := json.Marshal(spec)
	if err != nil {
		return types.MCPServer{}, fmt.Errorf("mcp_servers.Upsert marshal: %w", err)
	}
	var s types.MCPServer
	err = r.pool.QueryRow(ctx, `
		...
		Scan(&s.ID, &s.Name, &s.Transport, &s.Enabled, &metaScanner{&s.Spec}, &s.CreatedAt, &s.UpdatedAt)
	if err != nil {
		return types.MCPServer{}, fmt.Errorf("mcp_servers.Upsert: %w", err)
	}
	return s, nil
}
