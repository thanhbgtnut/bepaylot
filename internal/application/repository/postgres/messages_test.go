...
func TestAppendStoresHostileText(t *testing.T) {
	...
	msg := types.Message{SessionID: sess.ID, Role: types.RoleAssistant, Blocks: []types.ContentBlock{
		{Type: types.BlockText, Text: "Chào 5\xba tầng\x00"},
		{Type: types.BlockToolUse, ToolName: "http_fetch", ToolUseID: "t1", ToolInput: map[string]any{"url": "x\x00y", "n": int64(9007199254740993)}},
		// The reported failure: a tool result with a byte that is not UTF-8.
		{Type: types.BlockToolResult, ToolName: "http_fetch", ToolUseID: "t1", Text: "trang \xba cũ", ToolResult: map[string]any{"body": "\xba\x00"}},
	}}
...
func TestAppendConcurrentWritersGetDistinctSeq(t *testing.T) {
	...
	for i := 0; i < writers; i++ {
		go func(i int) {
			m := types.Message{SessionID: sess.ID, Role: types.RoleUser,
				Blocks: []types.ContentBlock{{Type: types.BlockText, Text: fmt.Sprint("m", i)}}}
...
func TestAgentRunDetailRoundTrips(t *testing.T) {
	...
	err = st.Runs.Insert(ctx, types.AgentRun{SessionID: sess.ID, Model: "m", Steps: 3, Detail: map[string]any{
		"stop_reason": "end_turn", "temperature": 0.2, "tool_calls": map[string]int{"calculate": 2}, "note": "a\x00b",
	}})
	if err != nil {
		t.Fatal(err)
	}
	if err := st.Runs.Insert(ctx, types.AgentRun{SessionID: sess.ID}); err != nil { // no detail at all
		t.Fatal(err)
	}
	var got string
