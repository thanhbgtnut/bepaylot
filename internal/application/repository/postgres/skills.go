...
func (r *SkillsRepo) Upsert(ctx context.Context, s types.Skill) (id uuid.UUID, changed bool, err error) {
...
func (r *SkillsRepo) List(ctx context.Context) ([]types.Skill, error) {
...
func (r *SkillsRepo) GetBySlug(ctx context.Context, slug string) (types.Skill, error) {
	rows, err := r.pool.Query(ctx, `
		SELECT id, slug, name, description, path, body, checksum, allowed_tools, enabled,
			(embedding IS NOT NULL) AS has_embedding, updated_at
		FROM skills WHERE slug = $1`, slug)
	if err != nil {
		return types.Skill{}, fmt.Errorf("skills.GetBySlug: %w", err)
	}
	defer rows.Close()
	...
	if len(list) == 0 {
		return types.Skill{}, ErrNotFound
	}
	return list[0], nil
}
