...
- **Schemas**: generated from the Go structs referenced in `@Param` / `@Success`
  / `@Failure` — these are the DTOs in `internal/handler/dto/`.

## How it is served

`internal/router/server.go` mounts (no auth):
...
`internal/router/routes_test.go` → `TestRoutesMatchOpenAPISpec` registers every
