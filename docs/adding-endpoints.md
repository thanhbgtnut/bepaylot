...
2. **Register the route** in `internal/router/server.go`.
...
Add a DTO if the response shape is new — put it in `internal/handler/dto/`:

```go
// internal/handler/dto/misc.go
...
The handler, in `internal/handler/skills.go`:
...
`internal/router/server.go`, inside the `v1` group:
...
- Reference a **named struct** from `internal/handler/dto` in `@Param`/`@Success`/
  `@Failure`. `swag` reads its `json` tags to build the schema.
- New response shape → add a struct to `internal/handler/dto/` (don't return
